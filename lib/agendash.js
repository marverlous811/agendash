...
module.exports = function(agenda, options) {
  ...
  const getJobs = (job, state, _limit, _offset, callback) => {
    const preMatch = {};
    if (job) {
      preMatch.name = job;
    }
    ...
    const limit = _limit || LIMIT_DEFAULT; // @TODO: UI param
    const skip = _offset || 0; // @TODO: UI param
  ...
  const api = function(job, state, limit = LIMIT_DEFAULT, offset = 0, callback) {
    if (!agenda) {
      return callback('Agenda instance is not ready');
    }
    async.parallel({
      overview: getOverview,
      jobs: getJobs.bind(this, job, state, limit, offset)
    },
    (err, apiResponse) => {
      if (err) {
